...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://sharkyswap.xyz/swap?inputCurrency=${token0Address}`
  } else {
    return `https://sharkyswap.xyz/swap?inputCurrency=${
      token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token0Address
...
export const urls = {
  showTransaction: (tx) => `https://arbiscan.io/tx/${tx}/`,
  showAddress: (address) => `https://arbiscan.io/address/${address}/`,
  showToken: (address) => `https://arbiscan.io/token/${address}/`,
  showBlock: (block) => `https://arbiscan.io/block/${block}/`,
}
